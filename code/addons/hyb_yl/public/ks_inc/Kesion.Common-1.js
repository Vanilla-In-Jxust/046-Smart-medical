...
function loadAjaxList(o) {
    ...
    if (id.substring(0, 7) == "Special") {
        ...
        if (typeid == 1) { //分页
            ...
            getlabelcontent("http://kv6.user.kesion.com/ks_inc/plus/ajaxLabel.aspx", id, labelid, "Label", typeid, channelid, classid, infoid);
        }
...
function Getcolor(obj, param) {
    ...
    $('#' + obj).attr('src', '../admin/images/rect.gif'/*tpa=http://kv6.user.kesion.com/admin/images/rect.gif*/);
};
...
digg = function(diggtype, channelid, infoid, installdir) {
    $.ajax({
        ...
        success: function(returnvalue) {
            switch (returnvalue) {
                case "err":
                    KesionJS.Message('error-1.gif'/*tpa=http://kv6.user.kesion.com/ks_inc/error.gif*/, 'digg error!', null);
                    break;
                case "over":
                    KesionJS.Message('error-1.gif'/*tpa=http://kv6.user.kesion.com/ks_inc/error.gif*/, '您已评分过了，不能重复评分！', null);
                    break;
                case "nologin":
                    KesionJS.Message('error-1.gif'/*tpa=http://kv6.user.kesion.com/ks_inc/error.gif*/, '您还没有登录，不能推荐!', null);
                    break;
                default:
                    try {
                        var sarr = returnvalue.split('|');
                        $('#s' + diggtype + sarr[0]).html(sarr[1 + diggtype]);
                        $('#d' + diggtype + sarr[0]).html("<a href='http://kv6.user.kesion.com/ks_inc/" + installdir + "index.aspx?c=model&a=dig' target='_blank'>查看</a>");
                    } catch (e) {}
                    try {
...
show_digg = function(diggtype, channelid, infoid, installdir) {
    $('#s' + diggtype + infoid).html("<img src='- + installdir + -sysimg/loading-1.gif'/*tpa=http://kv6.user.kesion.com/ks_inc/" + installdir + "sysimg/loading.gif*/ />");
    $.ajax({
        url: installdir + "plus/Ajaxs.ashx",
        cache: false,
...
var KesionJS = {
    ...
    AlertByTime: function(src, msg, t) {
        if (src == 1) {
            src = "success-1.gif"/*tpa=http://kv6.user.kesion.com/ks_inc/success.gif*/;
        } else {
            src = "error-1.gif"/*tpa=http://kv6.user.kesion.com/ks_inc/error.gif*/;
        }
        if (t == ''){ t = 2; }
    ...
    saveWordPic: function(callfrom, id) {
        ...
        var wordpic;
        try {
            wordpic = new ActiveXObject("http://kv6.user.kesion.com/ks_inc/Kesion.Word");
        } catch (exception) {
