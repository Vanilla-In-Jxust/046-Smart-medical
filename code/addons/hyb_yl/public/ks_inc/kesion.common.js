...
digg = function(diggtype, channelid, infoid, installdir) {
    $.ajax({
        ...
        success: function(returnvalue) {
            switch (returnvalue) {
                case "err":
                    KesionJS.Message('error.gif'/*tpa=http://kv6.user.kesion.com/KS_Inc/error.gif*/, 'digg error!', null);
                    break;
                case "over":
                    KesionJS.Message('error.gif'/*tpa=http://kv6.user.kesion.com/KS_Inc/error.gif*/, '您已评分过了，不能重复评分！', null);
                    break;
                case "nologin":
                    KesionJS.Message('error.gif'/*tpa=http://kv6.user.kesion.com/KS_Inc/error.gif*/, '您还没有登录，不能推荐!', null);
                    break;
                default:
                    try {
...
show_digg = function(diggtype, channelid, infoid, installdir) {
    $('#s' + diggtype + infoid).html("<img src='- + installdir + -sysimg/loading.gif'/*tpa=http://kv6.user.kesion.com/KS_Inc/" + installdir + "sysimg/loading.gif*/ />");
    $.ajax({
        url: installdir + "plus/Ajaxs.ashx",
        cache: false,
...
var KesionJS = {
    ...
    AlertByTime: function(src, msg, t) {
        if (src == 1) {
            src = "success.gif"/*tpa=http://kv6.user.kesion.com/KS_Inc/success.gif*/;
        } else {
            src = "error.gif"/*tpa=http://kv6.user.kesion.com/KS_Inc/error.gif*/;
        }
        if (t == ''){ t = 2; }
