	...
	*/
	...
	//查询缓存
 	static function getCache($key,$name) {
		...
		return cache_read('zfst:'.$_W['uniacid'].':'.$key.':'.$name);
	}
	...
	static function setCache($key,$name,$value) {
		global $_W;
		if(empty($key) || empty($name)) return false;	
		$res = cache_write('zfst:'.$_W['uniacid'].':'.$key.':'.$name,$value);
		return $res;
	}
	...
	static function deleteCache($key,$name) {
		global $_W;		
		if(empty($key) || empty($name)) return false;
		return cache_delete('zfst:'.$_W['uniacid'].':'.$key.':'.$name);
	}
	...
	static function dowebUrl($do,$data){
		global $_W;
		$str = '&do='.$do.'&op='.$data;
		if(!empty($array)){
			foreach($array as $k=>$v){
