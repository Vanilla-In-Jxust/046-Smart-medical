...
class circleSeal {
  ...
  public function __construct($str ='',$num = '',$rad = 100, $rmwidth = 6, $strad = 28, $stang = 0, $crang = 0, $fsize = 16, $inrad =0) {
    $this->sealString  = '印章测试字符串' ;
    $this->sealNum  = '010101010';
    $this->strMaxLeng  = 18;
    $this->sealRadius  = $rad;
    ...
    $this->font     = '../addons/hyb_yl/public/fonts/simhei.ttf';
    $this->fontSize   = $fsize;
  ...
  private function drawName1() {
    ...
    //相关计量
    $leng  = mb_strlen($this->sealName,'utf8');
    //字符串长度
    $nums = array();
    //拆分并写入字符串
    for ($i=0;$i<$leng;$i++) {
  ...
  private function drawNum() {
    ...
    //相关计量
    $this->numRadius = $this->sealRadius - $this->rimWidth - 5;
    //数字半径
    $leng  = mb_strlen($this->sealNum,'utf8');
    //字符串长度
    ...
    for ($i=0;$i<$leng;$i++) {
      ...
      $r =95 + $this->charAngle + $avgAngle*($i - $leng/2);
      //坐标角度
      $R = 720 - $this->charAngle + $avgAngle*($leng-2*$i-1)/2 ;
      //字符角度
      $x = $this->centerDot['x'] + $this->numRadius * cos(deg2rad($r));
      //字符的x坐标
      $y = $this->centerDot['y'] + $this->numRadius * sin(deg2rad($r));
      //字符的y坐标
  ...
  private function drawString() {
    ...
    //相关计量
    $this->charRadius = $this->sealRadius - $this->rimWidth - $this->fontSize-5;
    //字符串半径
    $leng  = mb_strlen($this->sealString,'utf8');
    //字符串长度
    if($leng > $this->strMaxLeng) $leng = $this->strMaxLeng;
    $avgAngle  = 270 / ($this->strMaxLeng);
    //平均字符倾斜度
    //拆分并写入字符串
    $words = array();
    //字符数组
    for ($i=0;$i<$leng;$i++) {
      ...
      $r = 630 + $this->charAngle + $avgAngle*($i - $leng/2);
      //坐标角度
      $R = 720 - $this->charAngle + $avgAngle*($leng-2*$i-1)/2 ;
      //字符角度
      $x = $this->centerDot['x'] + $this->charRadius * cos(deg2rad($r));
      //字符的x坐标
      $y = $this->centerDot['y'] + $this->charRadius * sin(deg2rad($r));
      //字符的y坐标
  ...
  private function drawString1() {
    ...
    //相关计量
    $charRadiusy = $this->centerDot['y'] - $this->rimWidth - $this->fontSize-2;
    //字符串y
    $charRadiusx = $this->centerDot['x'] - $this->rimWidth - $this->fontSize-2;
    //字符串x
    $leng  = mb_strlen($this->sealString,'utf8');
    //字符串长度
    if($leng > $this->strMaxLeng) $leng = $this->strMaxLeng;
    ...
    for ($i=0;$i<$leng;$i++) {
      ...
      $R = 720- $this->charAngle + $avgAngle*($leng-2*$i-1)/2;
      //字符角度
      $x = $this->centerDot['x'] + $charRadiusx * cos(deg2rad($r))+2;
      //字符的x坐标
      $y = $this->centerDot['y'] + $charRadiusy * sin(deg2rad($r));
      //字符的y坐标
      imagettftext($this->img, $this->fontSize, $R, $x, $y, $this->backGround, $this->font,$words[$i]);
    }
  }
  ...
  private function drawNum1() {
    ...
    //相关计量
    $charRadiusy = $this->centerDot['y'] - $this->rimWidth - $this->fontSize+15;
    //字符串y
    $charRadiusx = $this->centerDot['x'] - $this->rimWidth - $this->fontSize-2;
    //字符串x
    $leng  = mb_strlen($this->sealNum,'utf8');
    ...
    for ($i=0;$i<$leng;$i++) {
      $nums[] = mb_substr($this->sealNum,$i,1,'utf8');
      $r =97 + $this->charAngle + $avgAngle*($i - $leng/2);
      //坐标角度
      $R = 720 - $this->charAngle + $avgAngle*($leng-2*$i-1)/2 ;
      //字符角度
      $x = $this->centerDot['x'] + $charRadiusx * cos(deg2rad($r));
      //字符的x坐标
      $y = $this->centerDot['y'] + $charRadiusy * sin(deg2rad($r));
      //字符的y坐标
      imagettftext($this->img,10, $R, $x, $y, $this->backGround, $this->font, $nums[$i]);
  ...
  public function saveSquare($path,$name='') {
    ...
    // 创建一个正方形的图像
    $bgcolor     = imagecolorallocate($this->img, 255, 0, 0);
    // 图像的背景
    ...
    $img1    = imagecreatetruecolor(110, 110);
    // 创建一个正方形的图像
    $bgcolor     = imagecolorallocate($img1, 255, 255, 255);
    // 图像的背景
