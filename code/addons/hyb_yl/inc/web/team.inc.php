<?php 
	...
	if($op == 'shouyi')
	{
		...
		{
			$list = pdo_fetchall("select a.*,b.names,b.tel,c.u_thumb,c.u_id,c.u_name,c.randnum from ".tablename("hyb_yl_twenorder")." as a left join ".tablename("hyb_yl_userjiaren")." as b on a.j_id=b.j_id left join ".tablename("hyb_yl_userinfo")." as c on c.openid=a.openid ".$where." and a.ifpay != 0 and a.ifpay != 6 and a.ifpay != 8 group by a.back_orser order by a.id desc limit ".($pageindex - 1) * $pagesize.",".$pagesize);
			foreach($list as &$value)
			{
				$value['typs'] = '图文问诊';
		...
		}else if($server == 'tijian')
		{
			...
			$tuwen = pdo_fetchall("select a.*,b.names,b.tel,c.u_thumb,c.u_id,c.u_name,c.randnum from ".tablename("hyb_yl_twenorder")." as a left join ".tablename("hyb_yl_userjiaren")." as b on a.j_id=b.j_id left join ".tablename("hyb_yl_userinfo")." as c on c.openid=a.openid ".$where." and a.ifpay != 0 and a.ifpay != 6 and a.ifpay != 8 group by a.back_orser");
			...
			$goods = pdo_fetchall("select a.*,b.names,b.tel,c.u_thumb,c.u_id,c.u_name,c.randnum from ".tablename("hyb_yl_goodsorders")." as a left join ".tablename("hyb_yl_userjiaren")." as b on a.j_id=b.j_id left join ".tablename("hyb_yl_userinfo")." as c on c.openid=a.openid ".$where." and a.isPay=1");
...
//开卡记录
	if($op=="nkrecord"){
		...
		$list = pdo_fetchall("select p.*,u.u_name,u.u_thumb,u.u_phone from ".tablename("hyb_yl_card_pay")." as p left join ".tablename("hyb_yl_userinfo")." as u on u.openid=p.openid ".$where." order by p.id desc limit ".($pageindex - 1) * $pagesize.",".$pagesize);
		$total = pdo_fetchcolumn("select count(p.*) from ".tablename("hyb_yl_card_pay")." as p left join ".tablename("hyb_yl_userinfo")." as u on u.openid=p.openid ".$where);
		$pager = pagination($total, $pageindex, $pagesize);
		foreach($list as &$value)
		{
			...
			$value['keshi'] = pdo_getcolumn("hyb_yl_ceshi",array("id"=>$zhuanjia['parentid']),'name');
   			...
   			$card = pdo_get("hyb_yl_yearcard",array("id"=>$value['cid']));
   			$value['card'] = $card;
   			$value['p_time'] = date("Y-m-d",$value['p_time']);
   			$value['endtime'] = data("Y-m-d",$value['endtime']);
		}
