...
if($op == 'spreadrecord')
{
	$res = pdo_fetchall("SELECT a.*,b.u_id,b.u_name,b.u_thumb FROM".tablename('hyb_yl_tuikesite')."as a left join".tablename('hyb_yl_userinfo')."as b on b.openid=a.openid where a.uniacid=".$uniacid." and a.state=0");
...
{
	//查询所有人佣金
	$all = pdo_fetchall("SELECT a.id as aid ,a.*,b.id as bid,b.* FROM".tablename('hyb_yl_tuikeshouyi')."as a left join".tablename('hyb_yl_tuikesite')."as b on b.id =a.tkid where a.uniacid='{$uniacid}'  ");
    //来源
    if($all){
    	foreach ($all as $key => $value) {
    		$all[$key]['ly'] = pdo_get('hyb_yl_tuikesite',array('id'=>$value['xjid']),array('username'));
    	}
    }
