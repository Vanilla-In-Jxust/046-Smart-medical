<?php 
	...
	elseif ($_GPC['op'] == 'savepage') {
		...
		$tid = intval($_GPC['tid']);
		$temp = pdo_get('hyb_yl_temp',array('id'=>$tid,'uniacid'=>$_W['uniacid']));
		...
		if($id > 0){
			$res = pdo_update('hyb_yl_page',$data,array('id'=>$id));
		}else{
			$res = pdo_insert('hyb_yl_page',$data);
			$id = pdo_insertid();
		...
		$isset = pdo_get('hyb_yl_bar',array('uniacid'=>$_W['uniacid'],'tempid'=>$tid),array('id'));

		if( !empty( $isset['id'] ) ){
			$res = pdo_update('hyb_yl_bar',$data,array('id'=>$isset['id']));
		}else{
			$res = pdo_insert('hyb_yl_bar',$data);
			$id = pdo_insertid();
		}
		if($res){
	...
	// 查询链接
		...
		$page = pdo_getall('hyb_yl_page',array('uniacid'=>$_W['uniacid'],'tempid'=>$_GPC['tid']),array('id','name'));

		if( !empty( $page ) ){
			foreach ( $page as &$v ) {
				$v['url'] = '/hyb_yl/pages/page/page?pid='.$v['id'];
			}
			unset( $v );
		}
		
		$news = pdo_getall('hyb_yl_article',array('uniacid'=>$_W['uniacid']),array('id','title'));
		if( !empty( $news ) ){
			foreach ( $news as &$v ) {
				$v['url'] = '/hyb_yl/pages/art/art?aid='.$v['id'];
			}
		}
		...
		$temp = pdo_getall('hyb_yl_temp',array('uniacid'=>$_W['uniacid']),array('name','id'));

		if( !empty( $temp ) ) {
			foreach ($temp as &$v) {
				$v['page'] = pdo_getall('hyb_yl_page',array('tempid'=>$v['id']),array('id','name'));
			}
		}
		...
		pdo_update('hyb_yl_form',array('isread'=>1),array('id'=>$id,'uniacid'=>$_W['uniacid']));
		...
		if( $id > 0 ) {
			$res = pdo_update( 'hyb_yl_temp',$data ,array('id'=>$id,'uniacid'=>$_W['uniacid']));
		}else{
			$res = pdo_insert('hyb_yl_temp',$data);
		}
	...
	// 编辑模板
		...
		$temp = pdo_get('hyb_yl_temp',array('uniacid'=>$_W['uniacid'],'id'=>$_GPC['fid']));
	...
	// 使用模板
		...
		$id = intval( $_GPC['id'] );
		$temp = pdo_get('hyb_yl_temp',array('id'=>$_GPC['id']));
		if( empty( $temp ) ) Util::echoResult(201,'没有找到模板');

		if( $temp['issystem'] == 1 ) Util::echoResult(201,'不能使用系统模板');

		pdo_update('hyb_yl_temp',array('isact'=>0),array('uniacid'=>$_W['uniacid'],'issystem'=>0));
		pdo_update('hyb_yl_temp',array('isact'=>1),array('uniacid'=>$_W['uniacid'],'issystem'=>0,'id'=>$id));
		...
		if( $temp['name'] == '电镀厂系统模板' && $temp['issystem'] == 1 )	{
			...
			if( $tid ){
				// bar
				$bar = pdo_get('hyb_yl_bar',array('tempid'=>$temp['id']));
				$data = array(
					'uniacid' => $_W['uniacid'],
					'data' => $bar['data'],
					'createtime' => TIMESTAMP,
				...
				pdo_insert('hyb_yl_bar',$data);

				// page
				$allpage = pdo_getall('hyb_yl_page',array('tempid'=>$temp['id']));
				if( !empty( $allpage ) ) {
	...
	// 查询二维码
		...
		$res = Util::wxappQrcode( $uniacccount,'123456','hyb_yl/pages/form/form' );
	...
	// 普通模板转为系统模板
		...
		pdo_delete('hyb_yl_temp',array('id'=>$id));
		...
		$fid = intval( $_GPC['fid'] );
		if( $fid > 0 ){
			$form = pdo_get('hyb_yl_artsort',array('uniacid'=>$_W['uniacid'],'id'=>$fid));
		...
		if( $fid > 0 ){ 
			$res = pdo_update('hyb_yl_artsort',$data,array('id'=>$fid));
		}else{
			$res = pdo_insert('hyb_yl_artsort',$data);
		}
		if( $res ){
		...
		$form = pdo_get('hyb_yl_artsort',array('uniacid'=>$_W['uniacid'],'id'=>$_GPC['fid']));
		if( empty( $form ) ) Util::echoResult(201,'没有找到数据');
