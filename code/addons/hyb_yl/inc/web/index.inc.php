...
global $_W, $_GPC;
 ...
 { 
 	 ...
 	 public function adv()
     {
        ...
        $total = pdo_fetchcolumn("select * from ".tablename("hyb_yl_adv").$where);
   ...
   //底部菜单
    ...
    public function editmycenter()
    {
        ...
        if($_W['ispost'])
        {
            ...
            if($res)
            {
                if(!$pid)
                {
                    message("编辑成功",$this->createWebUrl('index',array('op'=>'mycenter','ac'=>'mycenter')),"success");
                }else{
                    message("编辑成功",$this->createWebUrl('index',array('op'=>'erjicenter','ac'=>'mycenter','pid'=>$pid)),"success");
                 
                }
            }else{
                if(!$pid)
                {
                    message("编辑失败",$this->createWebUrl('index',array('op'=>'mycenter','ac'=>'mycenter')),"success");
                }else{
                    message("编辑失败",$this->createWebUrl('index',array('op'=>'erjicenter','ac'=>'mycenter','pid'=>$pid)),"success");
    ...
    public function delmycenter()
    {
        ...
        if($res)
        {
            if($pid == '0')
            {   
                message("删除成功",$this->createWebUrl('index',array('op'=>'mycenter','ac'=>'mycenter')),"success");
            }else{
              message("删除成功",$this->createWebUrl('index',array('op'=>'erjicenter','ac'=>'mycenter')),"success");
            }
