<?php 
	...
	if(checksubmit('deleteall')){
		$res = WebCommon::deleteDataInWeb($_GPC['checkall'],'hyb_yl_form');
		message('操作完成,成功删除'.$res[0].'项，失败'.$res[1].'项',referer(),'success');
	}
	//批量删除
	if(checksubmit('deletealladmin')){
		$res = WebCommon::deleteDataInWeb($_GPC['checkall'],'hyb_yl_admin');
		message('操作完成,成功删除'.$res[0].'项，失败'.$res[1].'项',referer(),'success');
	}
	...
	if(checksubmit('readall')){
		...
		foreach ( $_GPC['checkall'] as $v ) {
			pdo_update('hyb_yl_form',array('isread'=>1),array('id'=>$v,'uniacid'=>$_W['uniacid']));
		}
	...
	if($_GPC['op'] == 'wait' || $_GPC['op'] == 'scaned'){
		...
		$info = Util::getAllDataInSingleTable('hyb_yl_form',$where,$_GPC['page'],$num,' `id` DESC ',false,true,' * ',$andstr);
		$list = $info[0];
		$pager = $info[1];
	...
	if($_GPC['op'] == 'deleteform'){
		$res = WebCommon::deleteSingleData($_GPC['id'],'hyb_yl_form');
	...
	/*if( $_GPC['op'] == 'admin' ){
		...
		$info = Util::getAllDataInSingleTable('hyb_yl_admin',array('uniacid'=>$_W['uniacid']),1,9999,' `id` DESC ',false,false);
		$list = $info[0];
		if( !empty( $list ) ) {
	...
	if($_GPC['op'] == 'deleteadmin'){
		$res = WebCommon::deleteSingleData($_GPC['id'],'hyb_yl_admin');
		if($res) message('删除成功',referer(),'success');
	}*/
