...
if($op == 'asklist')
{
    ...
    $where = "where a.uniacid = '{$uniacid}' and a.role=0 and a.grade=1";
    if($keywordtype=='1'){
    //订单号
        $where .=" ";
    }
...
if($op == 'telask')
{
    ...
    $where = " where a.uniacid = '{$uniacid}' and a.pid=0 and a.type=1 and a.keywords ='dianhuajizhen'";
    if($keywordtype == '1')
    {
...
if($op == 'squareaskdetails')
{
    $c_id = $_GPC['c_id'];
    $hid = $_GPC['hid'];
    $res = pdo_fetch("SELECT a.money as amoney,a.paytime as apaytime,a.*,b.money as bmoney,b.*,c.*,d.paytime as dpaytime,d.* FROM".tablename('hyb_yl_chufang')."as a left join".tablename('hyb_yl_userinfo')."as b on b.openid=a.useropenid left join".tablename('hyb_yl_userjiaren')."as c on c.j_id=a.userid and c.openid=b.openid left join".tablename("hyb_yl_goodsorders")."as d on d.cid=a.c_id where a.uniacid = '{$uniacid}' and a.c_id ='{$c_id}'  ");

    $res['doc_detail']=  pdo_fetch("select a.*,c.name from".tablename('hyb_yl_zhuanjia')."as a left join".tablename('hyb_yl_ceshi')."as c on c.id = a.parentid where a.uniacid='{$uniacid}' and a.zid='{$res['zid']}'");
   
    $back_orser = $res['orderNo'];
    $yizhu = unserialize($res['content']);
  
    $content = pdo_fetchall("select * from".tablename('hyb_yl_goodsorders')."where uniacid='{$uniacid}' and orderNo='{$back_orser}'");
...
if($op == 'videoask')
{
    ...
    $total = pdo_fetchcolumn("SELECT count(*) FROM".tablename('hyb_yl_wenzorder')."as a left join".tablename('hyb_yl_userjiaren')."as b on b.j_id =a.j_id left join".tablename('hyb_yl_userinfo')."as c on c.openid=a.openid left join".tablename('hyb_yl_zhuanjia')."as d on d.zid=a.zid".$where);
    $pager = pagination($total, $pageindex, $pagesize);
