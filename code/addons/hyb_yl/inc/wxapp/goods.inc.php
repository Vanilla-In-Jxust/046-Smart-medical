...
*/
 ...
 { 
   ...
   public function allgoods(){
      ...
      $row  = pdo_fetchall("SELECT a.*,b.*,c.detail FROM".tablename('hyb_yl_goodsarr')."as a left join ".tablename('hyb_yl_goodsfenl')."as b on b.fid=a.g_id left join".tablename('hyb_yl_yunfei')."as c on c.id=a.yf_id where a.uniacid='{$uniacid}' ORDER BY RAND() LIMIT 10");
   ...
   public function chufangdt(){
    ...
    public function creatorder(){
      ...
      // if(!empty($_GPC['back_orser'])){
      //    $orders  = $_GPC['back_orser'];
      // }else{
      ...
      if (!empty($chufangorderinfo) ) {
          $orders  = $chufangorderinfo['orders'];
      }else{
          $orders  = $this->getordernum();
      }
   ...
   //订单列表  
    ...
    public function upstatus(){
       ...
       $res = pdo_update("hyb_yl_goodsorders",array('orderStatus'=>$orderStatus),array('id'=>$id,'uniacid'=>$uniacid));
       echo json_encode($res);
    }
    //订单详情
