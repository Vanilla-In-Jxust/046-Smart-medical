...
* 
*/
 class Yaoqing extends HYBPage
 { 

public function addyihu()
  {
  global $_GPC, $_W;
  $zid = $_GPC['zid'];
 ...
 public function display(){
    ...
    $uniacid = $_W['uniacid'];
    $t_id =$_GPC['t_id'];
    $res = pdo_fetchall("SELECT * FROM".tablename("hyb_yl_yaoqingdoc")." as a left join".tablename("hyb_yl_zhuanjia")."as b on b.zid =a.zid where a.uniacid='{$uniacid}' and a.t_id='{$t_id}' and a.yao_type !=0 and a.yao_type !=2");
    foreach ($res as $key => $value) {
      ...
      if($res[$key]['yao_type'] ==3){
           $res[$key]['textname'] ="申请中";
      }
    }
 ...
 public function pass(){
  ...
  $uniacid = $_W['uniacid'];
  $yao_id =$_GPC['yao_id'];
  $res =pdo_update("hyb_yl_yaoqingdoc",array('yao_type'=>4),array('yao_id'=>$yao_id));
  echo json_encode($res);
 }

 public function dellyihu(){
  global $_GPC, $_W;
  $uniacid = $_W['uniacid'];
  $yao_id =$_GPC['yao_id'];
  $res =pdo_delete("hyb_yl_yaoqingdoc",array('yao_id'=>$yao_id));
  echo json_encode($res);
 }
 public function yaoqingjilu(){
  global $_GPC, $_W;
  $uniacid = $_W['uniacid'];
    $t_id =$_GPC['t_id'];
      $res = pdo_fetchall("SELECT * FROM".tablename("hyb_yl_yaoqingdoc")." as a left join".tablename("hyb_yl_zhuanjia")."as b on b.zid =a.zid where a.uniacid='{$uniacid}' and a.t_id='{$t_id}' and a.state=0");
      foreach ($res as $key => $value) {
        $res[$key]['yao_time']=date("Y-m-d H:i:s",$res[$key]['yao_time']);
        $res[$key]['z_thumbs'] =$_W['attachurl'].$res[$key]['z_thumbs'];
        if($res[$key]['yao_type'] ==0){
  ...
  public function entryyao(){
    ...
    public function join(){
       ...
       $get = pdo_get("hyb_yl_yaoqingdoc",array('zid'=>$zid,'t_id'=>$t_id,'yao_type'=>3));
       if(!$get){
           pdo_insert("hyb_yl_yaoqingdoc",$data);
           $res=array(
              'error' =>0,
              'text'  =>"提交成功"
          );
 ...
 public function detlte(){
  ...
  public function allteam(){
    ...
    public function upyihu(){
       ...
       pdo_update("hyb_yl_yaoqingdoc",$data,array('yao_id'=>$yao_id));
       $res = pdo_get("hyb_yl_zhuanjia",array('zid'=>$zid));
       $this->templatemsg($res['openid']);
    }
    public function templatemsg($openid){
      global $_GPC, $_W;
      $uniacid = $_W['uniacid'];
      //发送邀请模板消息给医生
      $wxappaid = pdo_get('hyb_yl_parameter', array('uniacid' => $uniacid));
