...
class Zhuanjia extends HYBPage {
  ...
  public function create_pic_watermark($dest_image,$watermark,$locate,$zid){
    ...
    public function alldocservertime() {
        ...
        $rwe = pdo_fetchall("SELECT * FROM" . tablename('hyb_yl_docjobtime') . "where uniacid='{$uniacid}' and zid ='{$zid}'");
        if(!$rwe)
        {
            $rwe = pdo_fetchall("SELECT * FROM" . tablename('hyb_yl_docjobtime') . "where uniacid='{$uniacid}' and zid =0");
        }
    ...
    public function dhorderlistser() {
        ...
        if($ifpay =='4'){
            $where ="where a.uniacid='{$uniacid}' and a.zid='{$zid}' and ifpay='6' and a.keywords='{$key_words}' and a.role=0  and a.type='1' order by a.id desc";
        }
...
//挂号
   ...
   public function addguahaozhuiwen() {
    ...
    public function getdocmbtxing() {
        ...
        $username =  preg_replace("/\\d+/",'', $user['z_name']); //专家名称
        $wxappaid = pdo_get('hyb_yl_parameter', array('uniacid' => $uniacid));
        $wxapp_mb = unserialize($wxappaid['wxapp_mb']);
   ...
   //开方接诊
    ...
    public function updatesporder() {
        ...
        $data = array('ifpay' => 2,'overtime'=>strtotime($overtime),'jz_time'=>strtotime($newtime));
    ...
    public function gettwdetail() {
        ...
        $uniacid = $_W['uniacid'];
        $id = $_GPC['id'];
        $res = pdo_get("hyb_yl_twenorder", array('id' => $id));
        $res['content'] = unserialize($res['content']);
        echo json_encode($res);
    }
    public function gettwzetail() {
        ...
        $uniacid = $_W['uniacid'];
        $id = $_GPC['id'];
        $res = pdo_get("hyb_yl_wenzorder", array('id' => $id));
        $res['content'] = unserialize($res['content']);
        echo json_encode($res);
    }
    public function chufangcont() {
        ...
        if(!empty($c_id) && $c_id!='undefined' ){
          $res = pdo_fetchall("SELECT a.*,b.advertisement,b.z_name from" . tablename('hyb_yl_goodsorders') . "as a left join" . tablename('hyb_yl_zhuanjia') . "as b on b.zid=a.zid where a.uniacid ='{$uniacid}' and a.cid='{$c_id}'");  
        }else{
           $res = pdo_fetchall("SELECT a.*,b.advertisement,b.z_name from" . tablename('hyb_yl_goodsorders') . "as a left join" . tablename('hyb_yl_zhuanjia') . "as b on b.zid=a.zid where a.uniacid ='{$uniacid}' and a.orderNo='{$back_orser}'");  
        }
    ...
    public function teamlists()
    {
        ...
        if($type == '0')
        {
            $list = pdo_fetchall("select t.*,z.z_zhicheng from ".tablename("hyb_yl_team")." as t left join ".tablename("hyb_yl_zhuanjia")." as z on z.zid=t.zid left join ".tablename("hyb_yl_hospital")." as h on h.hid=z.qx_id ".$where." order by t.id desc limit ".$page * $pagesize .",".$pagesize);

        }else if($type == '1')
        {
            $list = pdo_fetchall("select t.*,p.add_time,z.z_zhicheng,p.status as add_status from ".tablename("hyb_yl_team_people")." as p left join ".tablename("hyb_yl_team")." as t on t.id=p.tid left join ".tablename("hyb_yl_zhuanjia")." as z on z.zid=p.y_zid ".$where1." order by p.add_time desc limit ".$page * $pagesize .",".$pagesize);
  ...
  // 判断专家是否有年卡
    ...
    public function is_yearcard()
    {
        ...
        $zid = $_GPC['zid'];
        $yid = $_GPC['yid'];
        $res = pdo_fetch("select * from ".tablename("hyb_yl_user_yearcard")." where zid=".$zid." and yid=".$yid." and uniacid=".$uniacid);
        if($res)
        {
    ...
    public function user_blyear()
    {
        ...
        }else if($res && $res['end_time'] > time())
        {
            $data = array(
                ...
                "end_time" => strtotime($res['end_time']."+$shijian year"),
            );
            if($money == '0.00' || $money == '0')
            {
                $data['status'] = '1';
    ...
    public function update_pay()
    {
        ...
        pdo_upodate("hyb_yl_hospital",array("money"=>$money1),array("hid"=>$hospital['hid']));
    }
  ...
  public function pay_money()
  {
    ...
    if($is_year || $is_year == 'true')   //  是否使用年卡
    {
      ...
      if($zhuanjia_year && $user_year)   // 专家级用户是否都开通年卡
      {
        ...
        else if(($key_words == 'tuwenwenzhen' || $key_words == 'dianhuajizhen' || $key_words == 'shipinwenzhen' || $key_words == 'yuanchengkaifang') && ($user_year['wz_num'] == '0' || $user_year['hh_num'] == '0') && $user_year['wz_zhekou'] != '0'){    //问诊免费次数用完，使用折扣
      ...
      else if($zhuanjia_year && !$user_year)    //专家开启年卡，用户没开启
      {
        ...
        if(intval($zhuanjia_year['num']) > intval($count_user) && !$isbl)      //  判断专家年卡库存是否为0
        {
            ...
            }else if(($key_words == 'tuwenwenzhen' || $key_words == 'dianhuajizhen' || $key_words == 'shipinwenzhen' || $key_words == 'yuanchengkaifang') && ($zhuanjia_year['wz_num'] == '0' || $zhuanjia_year['hh_num'] == '0') && $zhuanjia_year['wz_zhekou'] != '0'){    //问诊免费次数为零，使用折扣
              if($user_card)   //用户是否有可用优惠全
              {
                $data = array(
  ...
  public function gh_order()
  {
    ...
    if($status == '4'){
        $where .= " and (g.ifpay=8 or g.ifgb=1) ";
    }
    ...
    // 查询详细明细
      ...
      public function getdocerweima()
      {
        ...
        if($erweima == '')
        {
          ...
          $access_token = $tokenArr->access_token;
          $tkid = !empty($_GPC['tkid'])?$_GPC['tkid']:'0';
          $mytkid = !empty($_GPC['mytkid'])?$_GPC['mytkid']:'0';

          $data = array();
          $data['scene'] = "zid=".$res['zid']."&tkid=".$tkid."&mytkid=".$mytkid;
          $data['page'] = "hyb_yl/czhuanjiasubpages/pages/zhuanjiazhuye/zhuanjiazhuye";
          $data = json_encode($data);
          $url = "https://api.weixin.qq.com/wxa/getwxacodeunlimit?access_token=" . $access_token;
          $result = $this->api_notice_increment($url, $data);
