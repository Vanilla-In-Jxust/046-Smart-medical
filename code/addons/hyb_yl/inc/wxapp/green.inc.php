 ...
 { 
        ...
        if($time != '0' || $time != '')
        {
          ...
          if($next < time())
          {
            ...
            }else if($zid)
            {
              ...
              }else if($zhuanjia['total_money'] < $moneys)
              {
                $data = array(
                    'code' => '1',
                    'message' => '余额不足，需扣除手续费'.$shouxufei.'元',
                ...
                $moneyss = $zhuanjia['total_money'] - $moneys;
  ...
  public function tixian_list()
  {
    ...
    if($did)
    {
      ...
      $list = pdo_fetchall("select a.*,b.z_name as title,b.advertisement as thumb from ".tablename("hyb_yl_pay")." as a left join ".tablename("hyb_yl_zhuanjia")." as b on b.zid=a.zid where a.uniacid=".$uniacid." and a.zid=".$zid." and a.style=0 and a.type=1 order by a.id desc limit ".$page * $pagesize.",".$pagesize);
    }
    foreach($list as &$value)
    {
      $value['thumb'] = tomedia($value['thumb']);
