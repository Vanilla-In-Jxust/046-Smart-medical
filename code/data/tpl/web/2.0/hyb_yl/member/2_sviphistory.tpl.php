...
<ul class="nav nav-tabs" id="myTab">
	<li class="active"><a href="#">购买记录</a></li>
</ul>
	...
	<div class="app-filter">
		<div class="filter-list">
			<form action="" method="get" class="form-horizontal" role="form" id="form1">
				...
				<input type="hidden" name="op" value="sviphistory" />
				<input type="hidden" name="ac" value="sviphistory" />
	...
	<div class="app-table-list">
		<div class="panel-body table-responsive collapse in" id="order-template-item-4" style="padding: 0;">
			<table class="table table-bordered">
				...
				<td style="text-align: center;">
					<img src="<?php  echo $item['u_thumb'];?>" class="scrollLoading" data-url="<?php  echo $item['thumb'];?>" height="50" width="50" ><?php  echo $item['u_name'];?>
				</td>
				<td style="text-align: center;"><?php  if(empty($item['u_phone'])) { ?>无<?php  } else { ?><?php  echo $item['u_phone'];?><?php  } ?></td>
				<td style="text-align: center;"><?php  echo $item['title'];?></td>
				<td style="text-align: center;"><?php  echo $item['times'];?>天</td>
				<td style="text-align: center;"><?php  echo $item['price'];?>元</td>
				<td style="text-align: center;"><?php  if($item['status'] == '0') { ?>待付款<?php  } else if($item['status'] == '1') { ?>已付款<?php  } ?></td>
				<td style="text-align: center;"><?php  echo $item['p_time'];?></td>
				</tr>
				<?php  } } ?>
				</tbody>
			</table>
...
<script type="text/javascript">
	...
	$('#de1').delegate('.pass_delete','click',function(e){
		...
		if(!state)  return;
			$.post("./index.php?c=site&a=entry&do=member&op=sviphistorydelete&ac=sviphistory&m=hyb_yl", { ids : ids }, function(data){
				if(data.errno=='1'){ 
					util.tips("操作成功！");
					setTimeout(function(){ 
