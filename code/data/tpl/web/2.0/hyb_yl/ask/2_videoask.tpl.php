	...
	<div class="app-content">
		<div class="app-filter">
			<div class="alert alert-warning">
				...
				<br />
				注意二：视频问诊非即时通讯，接诊则需要等待专家接诊
				<br />
			</div>
			<div class="filter-action">
				当前订单:<span class="cored"><?php  echo $count_order;?></span>单，共计问诊人数<span class="cored"><?php  echo $count_person;?></span>人，<span class="cored"><?php  echo $money_count;?></span>元;
			</div>
			<div class="filter-list">
				<form action="" method="post" class="form-horizontal" role="form" id="form1">
					<div class="form-group" style="max-width: 1180px;">
						...
						<div class="col-sm-10">
							<div class="btn-group">
								<a href="<?php  echo $this->createWebUrl('ask',array('op'=>'videoask','ifpay'=>'','start'=>$start,'end'=>$end,'keywordtype'=>$keywordtype,'keyword'=>$keyword,'pay_type'=>$pay_type,'ac'=>'videoask','hid'=>$_SESSION['hid']))?>" class="btn <?php  if($ifpay == '') { ?> btn-primary <?php  } else { ?> btn-default <?php  } ?>">不限</a>
								<a href="<?php  echo $this->createWebUrl('ask',array('op'=>'videoask','ifpay'=>'0','start'=>$start,'end'=>$end,'keywordtype'=>$keywordtype,'keyword'=>$keyword,'pay_type'=>$pay_type,'ac'=>'videoask','hid'=>$_SESSION['hid']))?>" class="btn <?php  if($ifpay == '0') { ?> btn-primary <?php  } else { ?> btn-default <?php  } ?>">待支付</a>
								
								<a href="<?php  echo $this->createWebUrl('ask',array('op'=>'videoask','ifpay'=>'1','start'=>$start,'end'=>$end,'keywordtype'=>$keywordtype,'keyword'=>$keyword,'pay_type'=>$pay_type,'ac'=>'videoask','hid'=>$_SESSION['hid']))?>" class="btn <?php  if($ifpay == '1') { ?> btn-primary <?php  } else { ?> btn-default <?php  } ?>">待接诊</a>

								<a href="<?php  echo $this->createWebUrl('ask',array('op'=>'videoask','ifpay'=>'2','start'=>$start,'end'=>$end,'keywordtype'=>$keywordtype,'keyword'=>$keyword,'pay_type'=>$pay_type,'ac'=>'videoask','hid'=>$_SESSION['hid']))?>" class="btn <?php  if($ifpay == '2') { ?> btn-primary <?php  } else { ?> btn-default <?php  } ?>">接诊中</a>
								<a href="<?php  echo $this->createWebUrl('ask',array('op'=>'videoask','ifpay'=>'3','start'=>$start,'end'=>$end,'keywordtype'=>$keywordtype,'keyword'=>$keyword,'pay_type'=>$pay_type,'ac'=>'videoask','hid'=>$_SESSION['hid']))?>" class="btn <?php  if($ifpay == '3') { ?> btn-primary <?php  } else { ?> btn-default <?php  } ?>">已完成/待评价</a>

								<a href="<?php  echo $this->createWebUrl('ask',array('op'=>'videoask','ifpay'=>'5','start'=>$start,'end'=>$end,'keywordtype'=>$keywordtype,'keyword'=>$keyword,'pay_type'=>$pay_type,'ac'=>'videoask','hid'=>$_SESSION['hid']))?>" class="btn <?php  if($ifpay == '5') { ?> btn-primary <?php  } else { ?> btn-default <?php  } ?>">已评价</a>
							</div>
						</div>
					</div>
					<div class="form-group">
						...
						<div class="col-sm-9">
							<div class="btn-group">
								<a href="<?php  echo $this->createWebUrl('ask',array('op'=>'videoask','ifpay'=>$ifpay,'start'=>$start,'end'=>$end,'keywordtype'=>$keywordtype,'keyword'=>$keyword,'pay_type'=>'','ac'=>'videoask','hid'=>$_SESSION['hid']))?>" class="btn <?php  if($pay_type == '') { ?> btn-primary <?php  } else { ?> btn-default <?php  } ?>">不限</a>
								<a href="<?php  echo $this->createWebUrl('ask',array('op'=>'videoask','ifpay'=>$ifpay,'start'=>$start,'end'=>$end,'keywordtype'=>$keywordtype,'keyword'=>$keyword,'pay_type'=>'0','ac'=>'videoask','hid'=>$_SESSION['hid']))?>" class="btn <?php  if($pay_type == '0') { ?> btn-primary <?php  } else { ?> btn-default <?php  } ?>">微信支付</a>
								<a href="<?php  echo $this->createWebUrl('ask',array('op'=>'videoask','ifpay'=>$ifpay,'start'=>$start,'end'=>$end,'keywordtype'=>$keywordtype,'keyword'=>$keyword,'pay_type'=>'1','ac'=>'videoask','hid'=>$_SESSION['hid']))?>" class="btn <?php  if($pay_type == '1') { ?> btn-primary <?php  } else { ?> btn-default <?php  } ?>">余额支付</a>
							</div>
						</div>
					</div>
					...
					<div class="">
							...
							<div class='table-row table-top'>
								<div style="text-align: left;color: #8f8e8e;">
									<span style="font-weight: bold;margin-right: 10px;color: #2d2d31">
										<span class="label label-info ordertype">视频</span> <?php  echo $item['time'];?>
									</span>
	                    ...
	                    ">
								...
								<div class="" style="overflow:visible;margin-top: 40px; text-align: center;">
									...
									<a class="btn btn-primary btn-sm" href="/index.php?c=site&a=entry&do=ask&op=videoaskdetails&ac=videoask&m=hyb_yl&id=<?php  echo $item['id'];?>&hid=<?php  echo $_SESSION['hid'];?>" title="">查看详情</a>
									<?php  if($item['ifpay'] =='0') { ?>
									<a class="btn btn-info btn-sm" href="<?php  echo $this->createWebUrl('ask',array('id'=>$item['id'],'ac'=>'videoask','op'=>'entervideoorder','hid'=>$_SESSION['hid']))?>">确定付款</a>
                                    <?php  } ?>
									<a class="btn btn-info btn-sm" href="/index.php?c=site&a=entry&do=ask&op=askchat&ac=videoask&m=hyb_yl&back_orser=<?php  echo $item['back_orser'];?>&keyword=shipinwenzhen&hid=<?php  echo $_SESSION['hid'];?>">快速回复</a>
									<a class="btn btn-danger btn-sm" data-toggle="ajaxRemove" href="/index.php?c=site&a=entry&do=ask&op=deleteshipin&ac=asklist&m=hyb_yl&back_orser=<?php  echo $item['back_orser'];?>&hid=<?php  echo $_SESSION['hid'];?>" data-confirm="确定要删除该记录吗？">快速删除</a>
