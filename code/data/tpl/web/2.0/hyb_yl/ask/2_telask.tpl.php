	...
	<div class="app-content">
		<div class="app-filter">
			<div class="alert alert-warning">
				...
				<br />
				注意二：电话问诊非即时通讯，接诊则需要等待专家接诊
				<br />
			</div>
			<div class="filter-action">
				当前订单:<span class="cored"><?php  echo $count_order;?></span>单，共计问诊人数<span class="cored"><?php  echo $count_person;?></span>人，<span class="cored"><?php  echo $count_money_or;?></span>元;
			</div>
			<div class="filter-list">
				<form action="" method="post" class="form-horizontal" role="form" id="form1">
					<div class="form-group" style="max-width: 1180px;">
						...
						<div class="col-sm-10">
							<div class="btn-group">
								<a href="<?php  echo $this->createWebUrl('ask',array('op'=>'telask','ifpay'=>'','start'=>$start,'end'=>$end,'keywordtype'=>$keywordtype,'keyword'=>$keyword,'pay_type'=>$pay_type,'ac'=>'telask','hid'=>$_SESSION['hid']))?>" class="btn <?php  if($ifpay == '') { ?> btn-primary <?php  } else { ?> btn-default <?php  } ?>">不限</a>
								<a href="<?php  echo $this->createWebUrl('ask',array('op'=>'telask','ifpay'=>'0','start'=>$start,'end'=>$end,'keywordtype'=>$keywordtype,'keyword'=>$keyword,'pay_type'=>$pay_type,'ac'=>'telask','hid'=>$_SESSION['hid']))?>" class="btn <?php  if($ifpay == '0') { ?> btn-primary <?php  } else { ?> btn-default <?php  } ?>">待支付</a>

								<a href="<?php  echo $this->createWebUrl('ask',array('op'=>'telask','ifpay'=>'1','start'=>$start,'end'=>$end,'keywordtype'=>$keywordtype,'keyword'=>$keyword,'pay_type'=>$pay_type,'ac'=>'telask','hid'=>$_SESSION['hid']))?>" class="btn <?php  if($ifpay == '1') { ?> btn-primary <?php  } else { ?> btn-default <?php  } ?>">待接诊</a>

								<a href="<?php  echo $this->createWebUrl('ask',array('op'=>'telask','ifpay'=>'2','start'=>$start,'end'=>$end,'keywordtype'=>$keywordtype,'keyword'=>$keyword,'pay_type'=>$pay_type,'ac'=>'telask','hid'=>$_SESSION['hid']))?>" class="btn <?php  if($ifpay == '2') { ?> btn-primary <?php  } else { ?> btn-default <?php  } ?>">接诊中</a>
								<a href="<?php  echo $this->createWebUrl('ask',array('op'=>'telask','ifpay'=>'3','start'=>$start,'end'=>$end,'keywordtype'=>$keywordtype,'keyword'=>$keyword,'pay_type'=>$pay_type,'ac'=>'telask','hid'=>$_SESSION['hid']))?>" class="btn <?php  if($ifpay == '3') { ?> btn-primary <?php  } else { ?> btn-default <?php  } ?>">已完成/待评价</a>

								<a href="<?php  echo $this->createWebUrl('ask',array('op'=>'telask','ifpay'=>'5','start'=>$start,'end'=>$end,'keywordtype'=>$keywordtype,'keyword'=>$keyword,'pay_type'=>$pay_type,'ac'=>'telask','hid'=>$_SESSION['hid']))?>" class="btn <?php  if($ifpay == '5') { ?> btn-primary <?php  } else { ?> btn-default <?php  } ?>">已评价</a>
							</div>
						</div>
					</div>
					<div class="form-group">
						...
						<div class="col-sm-9">
							<div class="btn-group">
								<a href="<?php  echo $this->createWebUrl('ask',array('op'=>'telask','ifpay'=>$ifpay,'start'=>$start,'end'=>$end,'keywordtype'=>$keywordtype,'keyword'=>$keyword,'pay_type'=>'','ac'=>'telask','hid'=>$_SESSION['hid']))?>" class="btn <?php  if($pay_type == '') { ?> btn-primary <?php  } else { ?> btn-default <?php  } ?>">不限</a>
								<a href="<?php  echo $this->createWebUrl('ask',array('op'=>'telask','ifpay'=>$ifpay,'start'=>$start,'end'=>$end,'keywordtype'=>$keywordtype,'keyword'=>$keyword,'pay_type'=>'0','ac'=>'telask','hid'=>$_SESSION['hid']))?>" class="btn <?php  if($pay_type == '0') { ?> btn-primary <?php  } else { ?> btn-default <?php  } ?>">微信支付</a>
								<a href="<?php  echo $this->createWebUrl('ask',array('op'=>'telask','ifpay'=>$ifpay,'start'=>$start,'end'=>$end,'keywordtype'=>$keywordtype,'keyword'=>$keyword,'pay_type'=>'1','ac'=>'telask','hid'=>$_SESSION['hid']))?>" class="btn <?php  if($pay_type == '1') { ?> btn-primary <?php  } else { ?> btn-default <?php  } ?>">余额支付</a>
							</div>
						</div>
					</div>
					...
					<div class="">
							...
							<div class='table-row table-top'>
								<div style="text-align: left;color: #8f8e8e;">
									<span style="font-weight: bold;margin-right: 10px;color: #2d2d31">
										<span class="label label-info ordertype">电话</span> <?php  echo $item['time'];?>
									</span>
								...
								<!-- 	<a class='op' data-toggle='ajaxPost' href="<?php  echo $this->createWebUrl('ask',array('op'=>'telaskdetails','id'=>$id))?>" data-confirm="确认生成相关分销订单？">
	                    ...
	                    ">
								...
								<div class="list-inner" style='text-align:center'>
									<span class="label label-warning"><?php  echo $item['zhuangtai'];?></span>
								</div>


								<div class="" style="overflow:visible;margin-top: 40px; text-align: center;">

									<a class="btn btn-primary btn-sm" href="<?php  echo $this->createWebUrl('ask',array('op'=>'telaskdetails','id'=>$item['id'],'ac'=>'telask','hid'=>$_SESSION['hid']))?>" title="">查看详情</a>
									<?php  if($item['ifpay'] =='0') { ?>
									<a class="btn btn-info btn-sm" href="<?php  echo $this->createWebUrl('ask',array('id'=>$item['id'],'ac'=>'telask','op'=>'entertelorder','hid'=>$_SESSION['hid']))?>">确定付款</a>
                                    <?php  } ?>
									<a class="btn btn-info btn-sm" href="/index.php?c=site&a=entry&do=ask&op=askchat&ac=telask&m=hyb_yl&back_orser=<?php  echo $item['back_orser'];?>&keyword=dianhuajizhen,'hid'=>$_SESSION['hid']">快速回复</a>
									<a class="btn btn-danger btn-sm" data-toggle="ajaxRemove" href="<?php  echo $this->createWebUrl('ask',array('op'=>'telaskdel','id'=>$item['id'],'ac'=>'telask','hid'=>$_SESSION['hid']))?>" data-confirm="确定要删除该记录吗？">快速删除</a>
