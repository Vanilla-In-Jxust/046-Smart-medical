...
<div class="app-content">
    ...
    <div class="app-filter">
        ...
        <div class="filter-action">
            <a class="btn btn-primary" href="/index.php?c=site&a=entry&do=team&op=schedulingadd&ac=schedulingadd&m=hyb_yl&zid=<?php  echo $zid;?>&hid=<?php  echo $_SESSION['hid'];?>">新增排班模板</a>
        </div>
                ...
                <?php  if(is_array($res)) { foreach($res as $item) { ?>
                    <tr>
                        ...
                        <td style="position: relative;">
                            <a class="btn btn-default btn-sm" href="<?php  echo $this->createWebUrl('team',array('op'=>'schedulingadd','id'=>$item['id'],'zid'=>$zid,'hid'=>$_SESSION['hid']))?>" title="编辑">编辑</a>

                            <a class="btn btn-danger btn-sm" href="<?php  echo $this->createWebUrl('team',array('op'=>'schedulingdel','id'=>$item['id'],'zid'=>$zid,'hid'=>$_SESSION['hid']))?>" data-toggle="ajaxRemove" data-confirm="确定要删除吗？" title="删除">删除</a>
                        </td>
                    </tr>
                    <?php  } } ?>
                </tbody>
    ...
    var enabled = "1";
                        $('#de1').delegate('.pass','click',function(e){
                            ...
                            if(!state) return;
                                ...
                                $.post("<?php  echo $this->createWebUrl('team',array('op'=>'schedulingdels'))?>", { ids : ids ,type:type}, function(data){
                                    if(!data.errno){
                                    util.tips("删除成功！");
                                    location.reload();
                                    }else{
