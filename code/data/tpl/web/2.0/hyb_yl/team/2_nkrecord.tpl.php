		...
		<div class="panel-body table-responsive collapse in" id="order-template-item-4" style="padding: 0;">
			<table class="table table-bordered">
				...
				<td>
					...
					<!--核销时间-->
					<td class="text-center" style="width:100px;font-family: "Arial","Microsoft YaHei","黑体","宋体",sans-serif;" >
					<?php  echo $item['card']['money'];?>元
					</td>
					<td class="text-center" style="width:100px;font-family: "Arial","Microsoft YaHei","黑体","宋体",sans-serif;" >
										<span class="label label-danger"><?php  if($item['end_time'] >= time()) { ?>已到期<?php  } else if($item['status'] == '0') { ?>待付款<?php  } else if($status == '1') { ?>使用中<?php  } ?></span>					</td>
					<!--验证码-->
					<td>
							支付时间：<?php  echo $item['p_time'];?><br>
	...
	$(function(){
		...
		$('#de1').delegate('.js-delete','click',function(e){
			...
			$checks.each(function() {
				...
				if(!state) return;
					$.post("https://www.webstrongtech.com/web/index.php?c=site&a=entry&m=hyb_yl&p=halfcard&ac=halfcard_web&do=deleteHalfcardRecord&", { ids : ids }, function(data){
						if(!data.errno){
						util.tips("删除成功！");
						location.reload();
						};
