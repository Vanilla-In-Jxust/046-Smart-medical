				...
				<?php  if(is_array($type)) { foreach($type as $typs) { ?>
					<div class="dashb-c-p">
						...
						<div class="dashb-c-p-r">
							<div class="h2">新增<?php  echo $typs['title'];?>数(人)</div>
							<div class="h1" id="html-allmoney-today"><?php  echo $typs['today'];?></div>
							<div class="p" id="html-allmoney-yestoday">昨日：<?php  echo $typs['yesterday'];?></div>
						</div>
					</div>
				<?php  } } ?>
                ...
                <div class="dashb-c-t1-c">
                    <div class="item">
                        ...
                        <div class="item_body">
                            ...
                            <a href=""><div class="item_info">待支付订单：<span id="order-dtk"><?php  echo $pay_order;?></span></div></a>
							<a href=""><div class="item_info">已结束订单：<span id="order-sqtk"><?php  echo $over_order;?></span></div></a>
                        </div>
                    </div>                
                </div>
			</div>
			...
			<div class="dashb-c-t2 dashb-c-top">
				...
				<div class="dashb-c-t2-c">
					<div class="dashb-c-p">
						<div class="dashb-c-p-r">
							...
							<div class="h1" id="html-allmoney"><?php  echo $today['pay_money'];?></div>
						</div>
					</div>
					<div class="dashb-c-p">
						<div class="dashb-c-p-r">
							<div class="h2">昨日支付金额(元)</div>
							<div class="h1" id="html-yesmoney"><?php  echo $yesterday['pay_money'];?></div>
						</div>
					</div>
					<div class="dashb-c-p">
						<div class="dashb-c-p-r">
							<div class="h2">近七日支付金额(元)</div>
							<div class="h1" id="html-sevenmoney"><?php  echo $seven['pay_money'];?></div>
						</div>
					</div>
					<div class="dashb-c-p">
						<div class="dashb-c-p-r">
							<div class="h2">近三十日支付金额(元)</div>
							<div class="h1" id="html-threemoney"><?php  echo $monthss['pay_money'];?></div>
						</div>
					</div>
				</div>
				<div class="page-content">
			        <form action=""  class="form-horizontal" onsubmit='return checkform()'>
			           ...
			           <input type="hidden" name="ac" value="green" />
        ...
        $('#container').highcharts({
            ...
            series: [{
	            ...
	            data: [
	                ...
	                <?php  if($key>0) { ?>,<?php  } ?><?php  echo $row['yuyuejiuzhen'];?>
	                <?php  } } ?>
	            ]
	        }, {
	            name: '住院安排',
	            data: [
	                <?php  if(is_array($datas)) { foreach($datas as $key => $row) { ?>
	                <?php  if($key>0) { ?>,<?php  } ?><?php  echo $row['zhuyuananpai'];?>
	                <?php  } } ?>
	            ]
	        }, {
	            name: '手术安排',
	            data: [
	                <?php  if(is_array($datas)) { foreach($datas as $key => $row) { ?>
	                <?php  if($key>0) { ?>,<?php  } ?><?php  echo $row['shoushuanpai'];?>
	                <?php  } } ?>
	            ]
	        }, {
	            name: '报告加急',
	            data: [
	                <?php  if(is_array($datas)) { foreach($datas as $key => $row) { ?>
	                <?php  if($key>0) { ?>,<?php  } ?><?php  echo $row['baogaojiaji'];?>
	                <?php  } } ?>
	            ]
	        }],
    	});
