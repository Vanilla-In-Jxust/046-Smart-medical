...
<div class="media media-wechat-setting">
	<a class="media-left">
		<span class="icon"><i class="wi wi-<?php  echo $account['type_sign'];?>"></i></span>
		<img src="<?php  echo $account['logo'];?>" class="wechat-img">
	</a>
	<div class="media-body media-middle ">
		<h4 class="media-heading color-dark"><?php  echo $account['name'];?></h4>
		<span class="color-gray"><?php  echo $account->typeName?></span>
	</div>
	<?php  if($state == ACCOUNT_MANAGE_NAME_FOUNDER || $state == ACCOUNT_MANAGE_NAME_OWNER) { ?>
	<div class="media-right media-middle">
		<a href="<?php  echo url('account/manage/delete', array('uniacid' => $account['uniacid'], 'account_type' => $account['type']))?>" class="btn btn-primary" onclick="return confirm('确认放入回收站吗？')">停  用</a>
	</div>
	<?php  } ?>
</div>
...
<div class="btn-group we7-btn-group wechat-edit-group">
	...
	<a href="<?php  echo url('account/post/base', array('uniacid' => $account['uniacid'], 'account_type' => $account['type']))?>" class="btn btn-default <?php  if($do == 'base') { ?> active<?php  } ?>">基础信息</a>
	<?php  } ?>

	<a href="<?php  echo url('account/post-user/edit', array('uniacid' => $account['uniacid'], 'account_type' => $account['type']))?>" class="btn btn-default <?php  if($action == 'post-user' && $do == 'edit') { ?> active<?php  } ?>">使用者管理</a>
	<a href="<?php  echo url('miniapp/manage/display', array('uniacid' => $account['uniacid'], 'account_type' => $account['type']))?>" class="btn btn-default <?php  if($action == 'manage' && $do == 'display') { ?> active<?php  } ?>">版本管理</a>
	<a href="<?php  echo url('account/post/modules_tpl', array('uniacid' => $account['uniacid'], 'account_type' => $account['type']))?>" class="btn btn-default <?php  if($action == 'post' && $do == 'modules_tpl') { ?> active<?php  } ?>">可用应用模板/模块</a>
	<?php  if($state == ACCOUNT_MANAGE_NAME_FOUNDER || $state == ACCOUNT_MANAGE_NAME_OWNER || $state == ACCOUNT_MANAGE_NAME_VICE_FOUNDER) { ?>
...
<div id="js-account-manage-aliapp" ng-controller="MiniappManageCtrl" ng-cloak>
	...
	<div class="text-right we7-margin-bottom">
		<a href="<?php  echo url('miniapp/post', array('uniacid' => $account['uniacid'], 'type' => $account['type']))?>" class="btn btn-primary" target="_blank">添加新版本</a>
	</div>
	<table class="table we7-table vertical-middle wxapp-version-table">
		<tr>
			<th class="text-left">版本号</th>
			<th class="text-left">应用</th>
			<?php  if($account['type_sign'] == 'wxapp') { ?>
		...
		<tr ng-if="version_exist" ng-repeat="item in version_lists">
			...
			<td class="text-center">
				...
				<div class="color-red" ng-if="item.last_modules.version != item.module.module_info.version">
	...
	<div class="modal fade" id="modal_edit_module"  tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
		<div class="modal-dialog we7-modal-dialog" style="width:800px">
			<div class="modal-content">
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
					<h4 class="modal-title">选择<?php  echo $account->typeName?></h4>
				</div>
...
require(['underscore'], function() {
	angular.module('wxApp').value('config', {
		...
		'links': {
			'edit_version': "<?php  echo url('miniapp/manage/edit_version', array('uniacid' => $account['uniacid'], 'account_type' => $account['type']))?>",
			'del_version': "<?php  echo url('miniapp/manage/del_version', array('uniacid' => $account['uniacid'], 'account_type' => $account['type']))?>",
		},
	});
