<?php defined('IN_IA') or exit('Access Denied');?><?php (!empty($this) && $this instanceof WeModuleSite) ? (include $this->template('/common/header-base', TEMPLATE_INCLUDEPATH)) : (include template('/common/header-base', TEMPLATE_INCLUDEPATH));?>

<style>
*{box-sizing: border-box;margin: 0;padding: 0;font-weight: 300;}
html,body{min-width: auto;min-height: auto;font-family: "微软雅黑";color: white;font-weight: 300;}
...
.container{width: auto;margin: 50px auto;text-align: center;z-index: 100;position: absolute;top: 40%;left: 50%;transform: translateX(-50%) translateY(-50%);}
...
<div class="wrapper">
	...
	<div class="container">
		...
		<h1><?php  if ($cloudset['name']) echo $cloudset['name'] ?>管理后台</h1>
		<form class="form" action="111" method="get" role="form">
			<input type="text" placeholder="请输入用户名登录" name="username" id="username">
			<input type="password" placeholder="请输入登录密码" name="password" id="password">
			<!-- <input type="submit" name="submit" value="提交" style="display: none;" id="form1">
			<button id="login-button">登录</button> -->
			<button id="login-button" type="button">登录</button>
			<!-- <button class="btn btn-primary btn-sml J-submit-btn" id="tijiao" type="submit">登录</button> -->
...
$('#login-button').click(function(event){
	...
	$.ajax({
		url:"<?php  echo $_W['siteroot'];?>/app/index.php?i=<?php  echo $_W['uniacid'];?>&c=entry&act=index&do=login&op=ispost&m=hyb_yl",
		data:{
			username:username,
			password:password
		},
		...
		success:function(res){
          ...
          console.log(hid)
			...
			{
				alert(res.message);
				window.location.href= urls+"/web/index.php?c=site&a=entry&do=dashboard&op=gk&m=hyb_yl&ac=dashboard&i=<?php  echo $uniacid;?>&hid="+hid;
			}else{
				alert(res.message);
			}
...
<!-- <div class="wrapper ">
	...
	<div class="container">
		...
		<form class="form" action="" method="post"  id="addForm">
			<input type="text" placeholder="请输入用户名登录" name="username" value="">
			<input type="password" placeholder="请输入登录密码" name="password" value="">
			<button id="login-button" type="button">登录</button>
		</form>
	</div>
...
$('#login-button').click(function(event){
     ...
     var url='https://www.webstrongtech.com/app/index.php?i=24&c=entry&act=index&do=login&m=hyb_yl';
      $.ajax({
        url:url,
        type: "POST",  
        dataType: "json",  
        ...
        success:function(res){
          ...
          if(res.message.errno=='0'){
          	// var hid = res.data;
          	// window.location.href = 'http://www.webstrongtech.com/web/index.php?c=site&a=entry&do=dashboard&op=gk&m=hyb_yl&ac=dashboard&hid='+hid;
          }
