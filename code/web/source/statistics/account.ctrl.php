...
$support_type = array(
	...
	if ('app_display' == $do) {
		...
		$today = !empty($today) ? (empty(current($today)['count']) ? 0 : current($today)['count']) : 0;
		$yesterday = stat_visit_all_bydate('yesterday', '', array(), true);
		$yesterday = !empty($yesterday) ? (empty(current($yesterday)['count']) ? 0 : current($yesterday)['count']) : 0;
