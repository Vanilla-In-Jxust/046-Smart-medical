...
!function() {
    ...
    r.prototype = {
        ...
        setupTypeNumber: function(t) {
            for (var e = l.getBCHTypeNumber(this.typeNumber), r = 0; r < 18; r++) o = !t && 1 == (e >> r & 1), 
        ...
        setupTypeInfo: function(t, e) {
            for (var r = this.errorCorrectLevel << 3 | e, o = l.getBCHTypeInfo(r), n = 0; n < 15; n++) i = !t && 1 == (o >> n & 1), 
        ...
        mapData: function(t, e) {
            for (var r = -1, o = this.moduleCount - 1, n = 7, i = 0, a = this.moduleCount - 1; a > 0; a -= 2) for (6 == a && a--; ;) {
                for (var s = 0; s < 2; s++) if (null == this.modules[o][a - s]) {
                    var h = !1;
                    i < t.length && (h = 1 == (t[i] >>> n & 1)), l.getMask(e, o, a - s) && (h = !h), 
        ...
        for (var n = i.getRSBlocks(t, e), s = new a(), h = 0; h < r.length; h++) {
            var u = r[h];
            s.put(u.mode, 4), s.put(u.getLength(), l.getLengthInBits(u.mode, t)), u.write(s);
        }
        var g = 0;
        for (h = 0; h < n.length; h++) g += n[h].dataCount;
        if (s.getLengthInBits() > 8 * g) throw new Error("code length overflow. (" + s.getLengthInBits() + ">" + 8 * g + ")");
        for (s.getLengthInBits() + 4 <= 8 * g && s.put(0, 4); s.getLengthInBits() % 8 != 0; ) s.putBit(!1);
        for (;!(s.getLengthInBits() >= 8 * g) && (s.put(o.PAD0, 8), !(s.getLengthInBits() >= 8 * g)); ) s.put(o.PAD1, 8);
        return o.createBytes(s, n);
    }, o.createBytes = function(t, e) {
        for (var r = 0, o = 0, i = 0, a = new Array(e.length), s = new Array(e.length), h = 0; h < e.length; h++) {
            var u = e[h].dataCount, g = e[h].totalCount - u;
            for (o = Math.max(o, u), i = Math.max(i, g), a[h] = new Array(u), p = 0; p < a[h].length; p++) a[h][p] = 255 & t.buffer[p + r];
            r += u;
            var f = l.getErrorCorrectPolynomial(g), c = new n(a[h], f.getLength() - 1).mod(f);
            for (s[h] = new Array(f.getLength() - 1), p = 0; p < s[h].length; p++) {
                var d = p + c.getLength() - s[h].length;
                s[h][p] = d >= 0 ? c.get(d) : 0;
            }
        }
        for (var m = 0, p = 0; p < e.length; p++) m += e[p].totalCount;
        var v = new Array(m), E = 0;
        for (p = 0; p < o; p++) for (h = 0; h < e.length; h++) p < a[h].length && (v[E++] = a[h][p]);
        for (p = 0; p < i; p++) for (h = 0; h < e.length; h++) p < s[h].length && (v[E++] = s[h][p]);
        return v;
    };
    for (var s = {
        MODE_NUMBER: 1,
        ...
        getErrorCorrectPolynomial: function(t) {
            for (var e = new n([ 1 ], 0), r = 0; r < t; r++) e = e.multiply(new n([ 1, g.gexp(r) ], 0));
            return e;
        },
    ...
    n.prototype = {
        ...
        multiply: function(t) {
            for (var e = new Array(this.getLength() + t.getLength() - 1), r = 0; r < this.getLength(); r++) for (var o = 0; o < t.getLength(); o++) e[r + o] ^= g.gexp(g.glog(this.get(r)) + g.glog(t.get(o)));
            return new n(e, 0);
        },
        mod: function(t) {
            if (this.getLength() - t.getLength() < 0) return this;
            for (var e = g.glog(this.get(0)) - g.glog(t.get(0)), r = new Array(this.getLength()), o = 0; o < this.getLength(); o++) r[o] = this.get(o);
            for (o = 0; o < t.getLength(); o++) r[o] ^= g.gexp(g.glog(t.get(o)) + e);
    ...
    i.getRSBlocks = function(t, e) {
        ...
        for (var o = r.length / 3, n = [], a = 0; a < o; a++) for (var s = r[3 * a + 0], h = r[3 * a + 1], u = r[3 * a + 2], l = 0; l < s; l++) n.push(new i(h, u));
        return n;
